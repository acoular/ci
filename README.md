...
- **Anaconda** uses an access token. The token needs to be generated on the [Anaconda website or CLI](https://www.anaconda.com/docs/tools/anaconda-org/admin-guide/tokens) (scope: "Allow all operations on Conda repositories"). Add it as `ANACONDA_TOKEN` to the `anaconda` environment of every publishing repository; it expires yearly and must be renewed.
